...
local function command(self, cmdstr, ...)
	...
	return function(...)
		...
		if shmt.proper_pipes then
			local input = { unpack(preargs.input or {}), unpack(args.input or {}) }
			unresolved[t] = { cmd = cmd, input = input }
		elseif is_5_2_plus then
			local input = (#preargs.input > 0 or #args.input > 0) and
				{ unpack(preargs.input or {}), unpack(args.input or {}) } or nil
			local msg
